...
/*
 * 
 * Фронтенд для получения прямых пополнений от терминалов 24NonStop в виде GET запроса
 * 
 */
 ...
 * 
 * @param $customerID - openpayz customer virtual ID
 * 
 * @return string
 */
 ...
 * 
 * @param $payId - payment ID without _prefix
 * @param $summ - payment summ
 * @param $customerid - customer virtual id
 * @param $note - transaction notes
 * 
 * @return  string
 */
function ns_PaymentProcessing($payId,$summ,$customerid,$note) {
    global $serviceName;
    $hash='24NS_'.  loginDB_real_escape_string($payId);
...
/*
 * function that checks transaction status by its payment ID
 * 
 * @param $payID
 * @return string
 */
...
if (ns_CheckGet(array('ACT', 'PAY_ID', 'SIGN'))) {
    ...
    if ($action == 1) {
        if (ns_CheckGet(array('PAY_ACCOUNT'))) {
            ...
            if (ns_CheckGet(array('TRADE_POINT'))) {
                $tradePoint = loginDB_real_escape_string($_GET['TRADE_POINT']);
                //check user availability and send reply
                die(ns_CheckCustomer($customerId));
            } else {
                die('TRADE_POINT FAILED');
            }
        } else {
                ...
                if (ns_CheckGet(array('RECEIPT_NUM'))) {
                    ...
                    if (ns_CheckGet(array('TRADE_POINT'))) {
                        ...
                        if ($checkSign) {
                            ...
                            $paymentHandler=ns_PaymentProcessing($payId, $summ, $customerId, 'term:'.$tradePoint.' receipt:'.$receiptNum);
                            die($paymentHandler);
                            } else {
                                die('SIGN ERROR');
                            }
