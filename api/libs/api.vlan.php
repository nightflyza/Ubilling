...
class RebootOnu {
	...
	if (!empty($this->onuData) AND ! empty($this->oltData) AND ! empty($this->oltSnmptemplate)) {
	    ...
	    $ifIndexFull	 = $this->snmp->walk($this->oltData['ip'], $this->oltData['snmp'], $ifIndexOid);
		...
		$this->snmp->set($this->oltData['ip'], $this->oltData['snmp'], $reloadData);
		return true;
	    }
...
class DescribeOnu {
	...
	if (!empty($this->onuData) AND ! empty($this->oltData) AND ! empty($this->oltSnmptemplate)) {
	    ...
	    $ifIndex	 = trim(str_replace(array($ifIndexOid, ' = INTEGER:'), '', $ifIndexFull));
	    if (!empty($ifIndex)) {
		$describeData[]	 = array('oid' => $snmpData['onu']['DESCRIPTION'] . '.' . $ifIndex, 'type' => 's', 'value' => '"' . $description . '"');
		$describeData[]	 = array('oid' => $snmpData['vlan']['SAVE'], 'type' => 'i', 'value' => '1');
		$checkResult	 = $this->snmp->set($this->oltData['ip'], $this->oltData['snmp'], $describeData);
		$checkResult.= $this->snmp->walk($this->oltData['ip'], $this->oltData['snmp'], $snmpData['onu']['DESCRIPTION'] . '.' . $ifIndex, FALSE);
...
class VlanGen {
	...
	$Desc		 = vf(DB_real_escape_string($Desc));
