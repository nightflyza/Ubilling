<?php
 ...
 function catv_UserProfileShow($userid) {
  ...
  function catv_AjaxOnlineDataSource() {
     ...
     $clearstreet=  DB_real_escape_string($clearstreet);
     
     $clearrealname=trim($eachuser['realname']);
     $clearrealname=  str_replace("'", '`', $clearrealname);
     $clearrealname=  DB_real_escape_string($clearrealname);
 ...
 function catv_UserSetTariffNM($userid,$tariffid) {
  ...
  function catv_UserEdit($userid,$contract,$realname,$street,$build,$apt,$phone,$discount,$decoder,$inetlink,$notes) {
                ...
                $realname=DB_real_escape_string($realname);
                $street=DB_real_escape_string($street);
                $build=DB_real_escape_string($build);
                $apt=DB_real_escape_string($apt);
                $phone=DB_real_escape_string($phone);
                $discount=DB_real_escape_string($discount);
                $decoder=DB_real_escape_string($decoder);
                $inetlink=DB_real_escape_string($inetlink);
    ...
    if (!empty ($allinprocessed)) {
        ...
        foreach ($allinprocessed as $io=>$eachrow) {
            //setting payment variables
             ...
             $cash=$eachrow['summ'];
             $note=DB_real_escape_string("CATV_BANKSTA:".$eachrow['id']);
