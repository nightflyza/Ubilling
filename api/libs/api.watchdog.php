    ...
    protected function doAction($taskID = NULL) {
        if (!empty($taskID)) {
            switch ($this->taskData[$taskID]['checktype']) {
                ...
                case 'getusertraff':
                    if (!empty($this->taskData[$taskID]['param'])) {
                        $userLoging = loginDB_real_escape_string($this->taskData[$taskID]['param']);
...
class WatchDogInterface {
    ...
    public function createTask($name, $checktype, $param, $operator, $condition, $action, $active = 0) {
        $active = loginDB_real_escape_string($active);
        $name = loginDB_real_escape_string($name);
        $checktype = loginDB_real_escape_string($checktype);
        $param = loginDB_real_escape_string($param);
        $operator = loginDB_real_escape_string($operator);
        $condition = loginDB_real_escape_string($condition);
        $action = loginDB_real_escape_string($action);
