            ...
            if (!empty($template)) {
                ...
                if (!empty($message)) {
                    foreach ($numbers as $number) {
                        ...
                        $queueFile = $this->sms->sendSMS($number, $message, false, 'REMINDER');
        ...
        foreach ($this->AllLogin as $userLoginData) {
            ...
            if ($onlineDaysLeft <= $liveDays and $onlineDaysLeft >= 0 and $this->rmdMode != 2 and empty($userLoginData['Passive'])) {
                ...
                if (!file_exists(self::FLAGPREFIX . $eachLogin)) {
                    ...
                    $this->debugReminder('CONSIDER BASE SERVICE', $eachLogin, $userLoginData['Cash'], print_r($numbers, true),
                                         $liveDays, $liveTime, $cacheTime, 'online days left: ' . $onlineDaysLeft);
                }
            }
                ...
                if (!file_exists(self::FLAGPREFIX . self::CREDITPREFIX . $eachLogin)) {
                    ...
                    if (!empty($creditSum) and empty($creditExpireTime)) {
                        ...
                        if ($remindStartTime <= $curTime and $creditExpireTime > $curTime) {
                            ...
                            $this->debugReminder('CONSIDER CREDIT', $eachLogin, $userLoginData['Cash'], print_r($numbers, true),
                                                 $liveDays, $liveTime, $cacheTime,
                                         'remind start time: ' . $remindStartTime . ' current time: ' . $curTime . ' credit expire time: ' . $creditExpireTime);
                        }
                    }
                }
            }
            ...
            if ($this->rmdConsiderCAP and $onlineDaysLeft == -1 and !empty($userLoginData['days']) and empty($userLoginData['Passive'])) {
                ...
                if (!file_exists(self::FLAGPREFIX . self::CAPPREFIX . $eachLogin)) {
                    ...
                    if ($capDaysLeft <= $liveDays) {
                        ...
                        $this->debugReminder('CONSIDER CAP', $eachLogin, $userLoginData['Cash'], print_r($numbers, true),
                                             $liveDays, $liveTime, $cacheTime, 'days till CAP left: ' . $capDaysLeft);
                    }
                }
            }
            ...
            if ($this->rmdConsiderFrozen and !empty($userLoginData['Passive'])
                ...
                if (!file_exists(self::FLAGPREFIX . self::FROZENPREFIX . $eachLogin)) {
                    ...
                    if ($freezeDaysLeft <= $liveDays) {
                        ...
                        $this->debugReminder('CONSIDER FROZEN', $eachLogin, $userLoginData['Cash'], print_r($numbers, true),
                                             $liveDays, $liveTime, $cacheTime, 'freeze days left: ' . $freezeDaysLeft);
                    }
                }
            }
