...
function em_EmployeeAdd($name, $job, $mobile = '', $telegram = '', $admlogin = '') {
    $name = loginDB_real_escape_string(trim($name));
    $job = loginDB_real_escape_string(trim($job));
    $mobile = loginDB_real_escape_string($mobile);
    $telegram = loginDB_real_escape_string($telegram);
    $admlogin = loginDB_real_escape_string($admlogin);
...
function stg_add_jobtype($jobtype, $jobcolor) {
    $jobtype = loginDB_real_escape_string(trim($jobtype));
    $jobcolor = loginDB_real_escape_string($jobcolor);
...
function stg_add_new_job($login, $date, $worker_id, $jobtype_id, $job_notes) {
    $job_notes = loginDB_real_escape_string(trim($job_notes));
...
function ts_CreateTask($startdate, $starttime, $address, $login, $phone, $jobtypeid, $employeeid, $jobnote) {
    ...
    $startdate = loginDB_real_escape_string($startdate);
    $jobSendTime = (!empty($starttime)) ? ' ' . date("H:i", strtotime($starttime)) : '';

    if (!empty($starttime)) {
        $starttime = "'" . loginDB_real_escape_string($starttime) . "'";
    } else {
        $starttime = 'NULL';
    }
    $jobtypeid = vf($jobtypeid, 3);
    $employeeid = vf($employeeid, 3);
    $jobnote = loginDB_real_escape_string($jobnote);
    ...
    if (!empty($taskdata)) {
        ...
        if (!empty($taskdata['smsdata'])) {
            ...
            if ($altercfg['SENDDOG_ENABLED']) {
                ...
                $smsAddress = loginDB_real_escape_string($smsAddress);
                $smsPhone = loginDB_real_escape_string($taskdata['phone']);
                $smsJobTime = (!empty($taskdata['starttime'])) ? ' ' . date("H:i", strtotime($taskdata['starttime'])) : '';
                $smsJobNote = loginDB_real_escape_string($taskdata['jobnote']);
